...
│   ├── (app)/settings/page.tsx  # тема, язык, muted-категории (динамические)
...
- **Inbox** — список писем, табы категорий (встроенные + пользовательские метки почты), переключение аккаунтов (включая добавление аккаунта внутри Mini App), открытие письма оверлеем без потери скролла/категории/аккаунта, живой статус прочитанного (оптимистичный mark-read + откат при ошибке), Load more
- **Message** — отправитель, тема, тело (sanitized rich HTML), кнопка «прочитано» с blob-анимацией; автоматически помечается прочитанным при долистывании до конца
- **Settings** — тема (белая/тёмная, мгновенный переключатель-пилюля), язык и muted-категории. Категории подгружаются с сервера при каждом открытии. Кнопка «Сохранить» неактивна, пока не внесены изменения. Информации об интервале синхронизации нет: он фиксированный, 10 секунд.
