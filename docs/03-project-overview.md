...
2. Единая reply-клавиатура «📬 Мои аккаунты» → «➕ Подключить» → Gmail/Яндекс → бот шлёт OAuth-ссылку.
...
4. Интервал у каждого аккаунта фиксированный: ровно 10 секунд с момента подключения; ошибки → экспоненциальный backoff.

Пользователь не выбирает частоту синхронизации: каждый аккаунт проверяется каждые 10 секунд, настройки интервала нет.
...
- Интервал синхронизации: фиксированный, 10 секунд; в настройках не показывается.
...
| Фиксированный интервал 10с | ✅ постоянный, без ручного выбора |
...
- Август 2026: деплой на VPS, инкрементальная синхронизация, анти-флуд уведомлений, фиксированный интервал 10с, mark-read с прокидыванием в провайдера, Telegram Mini App auth и внешние проверки по HTTPS-домену.
